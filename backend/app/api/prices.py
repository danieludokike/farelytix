...
def get_price_series(tracked_id: str, db: Session = Depends(get_db)):
    ...
    return {
        ...
        "series": series,
        "forecast": []  # will fill later
    }

# DEV helper: quickly generate a random snapshot (no scraper yet)
...
def add_random_snapshot(tracked_id: str, db: Session = Depends(get_db)):
    ...
    import random
    price_cents = random.randint(25000, 60000)  # 250–600 USD
    crud.add_price_snapshot(
        db,
        tracked_id=tracked_id,
        price_cents=price_cents,
        currency="USD",
        provider=search.provider or "DevProvider",
    )
